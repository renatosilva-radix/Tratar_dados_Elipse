...
def encontrar_arquivos(files, root):
    arquivos = {}
    for file in files:
...
def executar_scripts(diretorio_principal, arquivo_funcionalidades, arquivo_plantas_sistemas, diretorio):
    nome_planilha = "Equipamentos e Funcionalidades"
    # Executa o script `Criar Lista de Medidas.py` no diretório correto
    print(f"Executando script para {arquivo_funcionalidades} e {arquivo_plantas_sistemas} no diretório {diretorio}")
    subprocess.run(["python", os.path.join(diretorio_principal, "Criar Lista de Medidas.py"),
                    arquivo_funcionalidades, nome_planilha, diretorio],
                   cwd=diretorio)  # Define o diretório de trabalho para a execução do script
...
def main():
    ...
    for root, dirs, files in os.walk(diretorio_pre_python):
        arquivos = encontrar_arquivos(files, root)
        if 'funcionalidades' in arquivos and 'plantas_sistemas' in arquivos:
            ...
            # Converte os arquivos Excel para CSV
            ...
            # Executa o script `Criar Lista de Medidas.py`
            executar_scripts(diretorio_principal,
                             arquivos['funcionalidades'],
                             arquivos['plantas_sistemas'],
                             pasta_destino)
